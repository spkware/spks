...
def __par_init_waveforms(bin_file_path, waveform_mmap_path, mmap_shape, local_timestamps, local_time_indices):
    ...
    data, _ = load_spikeglx_binary(bin_file_path)
...
def _work_extract_waveforms(waveforms, chunk_inds, flush_memory=True):
...
def _tr_work_extract_waveforms(data, waveforms, timestamps, time_indices, chmap, chunk_inds, flush_memory=True):
    """Extracts waveforms from binary file and writes them to the global variable waveforms."""
...
def extract_memmapped_waveforms(binfile_path, scratch_directory, nchannels, timestamps, n_spikes_per_chunk=1000, npre=30, npost=30, chmap=None):
    ...
    mpfunc = partial(_work_extract_waveforms, tfile)
    ...
    #with ThreadPoolExecutor(max_workers=cpu_count(),
    #                        initializer=__par_init_waveforms,
    #                        initargs=(binfile_path, tfile.filename, mmap_shape, timestamps, time_indices)) as executor:
    #    _ = executor.map(mpfunc, chunk_inds)
...
def tr_extract_memmapped_waveforms(binfile_path, scratch_directory, nchannels, timestamps, n_spikes_per_chunk=1000, npre=30, npost=30, chmap=None):
    ...
    tfile.flush()
    print(f'\nWaveforms mapped to {tfile.filename}')
    mpfunc = partial(_tr_work_extract_waveforms, data, tfile, timestamps, time_indices, chmap)
